...
//======Private Functions======
uint16_t getFifoPtr(uint16_t current, uint16_t offset,const fifo_t* fifo)
{
    return (current + offset) % fifo->buffersize;
}

uint32_t min(uint32_t number1, uint32_t number2)
{
    if(number1 <= number2)
        return number1;
    else
...
{
    int64_t size;
    size = (int64_t)fifo->tail - (int64_t)fifo->head;
    if (size < 0)
    {
...
{
    if (fifo_empty(fifo) || number > fifo_datasize(fifo))
        return 0;

    return (void*) &fifo->buffer[fifo->head + number * objectSize];
}
